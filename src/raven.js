...
Raven.prototype = {
    ...
    _handleStackInfo: function(stackInfo, options) {
        ...
        if (stackInfo.stack && stackInfo.stack.length) {
            each(stackInfo.stack, function(i, stack) {
                var frame = this._normalizeFrame(stack);
                if (frame) {
                    frames.push(frame);
                }
            });
    ...
    _send: function(data) {
        var baseData = {
            ...
            logger: this.globalOptions.logger,
            platform: 'javascript'
        };
        var http = this._getHttpData();
        if (http) {
        ...
        data.extra = objectMerge({
            'session:duration': now() - this.startTime
        }, data.extra);
        ...
        // Include the release if it's defined in this.globalOptions
        if (this.globalOptions.release) data.release = this.globalOptions.release;

        if (isFunction(this.globalOptions.dataCallback)) {
            data = this.globalOptions.dataCallback(data) || data;
        }
        ...
        if (isFunction(this.globalOptions.shouldSendCallback) && !this.globalOptions.shouldSendCallback(data)) {
            return;
        }
        ...
        this.lastEventId = data.event_id || (data.event_id = uuid4());

        this._logDebug('debug', 'Raven about to send:', data);

        if (!this.isSetup()) return;

        (this.globalOptions.transport || this._makeRequest)({
    ...
    _logDebug: function(level) {
        if (this.originalConsoleMethods[level] && Raven.debug) {
...
//         RavenConfigError: RavenConfigError
        ...
        //         this.isRavenInstalled = state.isRavenInstalled;
        //     if ('hasJSON' in state)
        //         hasJSON = state.hasJSON;
        //     if ('globalProject' in state)
        //         this.globalProject = state.globalProject;
        //     if ('globalOptions' in state)
        //         this.globalOptions = state.globalOptions;
        //     if ('globalServer' in state)
        //         this.globalServer = state.globalServer;
        //     if ('globalContext' in state)
        //         this.globalContext = state.globalContext;
        // },

        // getGlobalState: function () {
        //     return {
        //         this.isRavenInstalled: this.isRavenInstalled,
        //         this.globalProject: this.globalProject,
        //         this.globalOptions: this.globalOptions,
        //         this.globalServer: this.globalServer,
        //         this.globalContext: this.globalContext,
